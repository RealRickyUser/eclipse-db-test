...
public class DeleteTestCategoryAction extends BaseConfirmSelectionListenerAction {

	public DeleteTestCategoryAction(String title) {
		super(title);
	...
	protected void doWork() {
		...
		List<Integer> cats = service.getAllTestCategories(cat.getId());
			...
			public void run(IProgressMonitor monitor) throws InvocationTargetException, InterruptedException {
				...
				for (Integer categoryId : cats) {
					if (monitor.isCanceled())
						return;
					service.deleteTestCategory(categoryId);
					monitor.worked(pos++);
				}
				monitor.done();
			}
