...
public class DeleteScriptCategoryAction extends BaseConfirmSelectionListenerAction {

	public DeleteScriptCategoryAction(String title) {
		super(title);
	...
	protected void doWork() {
		...
		List<Integer> cats = service.getAllCategories(cat.getId());
			...
			public void run(IProgressMonitor monitor) throws InvocationTargetException, InterruptedException {
				...
				for (Integer categoryId : cats) {
					if (monitor.isCanceled())
						return;
					service.deleteScriptCategory(categoryId);
					monitor.worked(pos++);
				}
				monitor.done();
			}
